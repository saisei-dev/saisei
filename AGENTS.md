- This is a JIT recompiler: no program code is decoded ahead of time. `saisei-cli build <name>` only generates the per-program GameConfig (Rust) and builds the game binary via cargo; all program code is JIT-compiled at run time into `build/<name>/jit/`. The whole toolchain is Rust; build it with `cargo build --release`.
...
- Use `saisei-cli run <name> --headless` to build and execute a program. A change is validated by the program reaching its known scene — screenshot it (`--screenshot-secs N`), don't rely on unit tests alone.
