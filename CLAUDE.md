...
**Per-game config (`games/<name>/<name>.json`):** `name`, `program_path` (the MZ image to load), optional `programs` (multi-executable bundles, each with its own `program_path`/`psp_seg`), `psp_seg`/`init_cs` (machine params), `protected_slots` (runtime memory-protection ranges), optional `args` (the **command tail** — what a user would have typed after the program's name; it lands at PSP:80h exactly as DOS leaves it, leading space and all), and `runtime` (files copied into `build/<game>/` at run). `args` is not a nicety: a DOS program's command line is part of *how it is invoked*, and passing an empty one runs a different command than the game shipped with — MechWarrior's `MW.EXE` picks its sound device by scanning its own tail for S/T/A/R and, given nothing, chooses a **Roland MT-32** this machine has no MPU-401 for. `MW A` is the invocation. Both consumers read it through `saisei::game_config_values` — `generate_game_config` bakes those values into a frozen per-game binary, and the player installs them at run time — so the two cannot drift. The per-binary `<binary>.json` sidecars and the `aliases`/`callgraph`/`regions`/`vars`/`enums` files are reverse-engineering annotations (function names, comments, discovered entries) — not part of the JIT run path. Diagnosis artifacts land in `build/<game>/` (`lifecycle.log`, `watchw.log`) and `crashes/`.
