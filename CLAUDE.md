...
- device emulation: `io_bus.rs`, `video.rs`, `keyboard.rs`, `timer.rs`; display: `sdl.rs`; persistence: `snapshot.rs`, `save_manager.rs` (their `#[repr(C)]` layouts are FROZEN — snapshots serialize them byte-for-byte).
