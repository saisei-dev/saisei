...
> NOTE: the toolchain is **Rust** and lives at the repo root: `saisei-jitc/` (the
> translator + JIT, exposed as the `saisei-jitc` binary + library) and
> `saisei/` (the `saisei` launcher). The C runtime stays in `runtime/` and
> is compiled with clang. There is no the reference in the repo.
...
Useful `run` flags: `--verbose` (runs are silent by default; this prints the shim trace to stdout), `--trace-file <path>` (write execution trace), `--lifecycle-file <path>` (stream LOAD/CALL/JMP/… events). The launcher sets `SAISEI_REPO_ROOT`/`SAISEI_JITC`/`SAISEI_JIT_DIR` itself (the runtime JIT needs them to invoke the `saisei-jitc` translator and cache chunks). Screenshots: pass `--screenshot-secs N` (headless runs) to auto-dump PNGs to `build/<name>/screenshots/`.

Inject compiler defines with `CFLAGS=…` — the `saisei` launcher forwards them to the runtime link (e.g. `CFLAGS=-DFORCE_EXIT_AFTER_10S saisei run <name> --headless` for a self-terminating smoke run).

Tests are Rust `cargo test` — the ported translator unit tests
(`saisei-jitc/tests/`, exercising `compiler`-equivalent translation logic)
and the `runtime/` shim tests (compiled per-test and driven via dlopen/FFI), plus
launcher tests in `saisei/tests/`. Set `SAISEI_CAPSTONE_LIB_DIR` to the
Capstone lib dir at build time.

```bash
cargo test                                              # whole workspace
cargo test -p saisei-jitc --test ported_disasm          # one ported test file
cargo test -p saisei-jitc --test ported_disasm disassemble_retf__retf  # one test
```

## Architecture

**Translator (`saisei-jitc/src/`, shared by every JIT compile — the Rust
port of the former `compiler/*.py`, validated byte-identical against it):**
...
- `ir_to_c.rs` — the translator. IR → basic blocks (`build_basic_blocks`) → CFG (`cfg.rs`, a insertion-ordered `graph.rs` DiGraph) → structured regions (`patterns.rs`, `ast.rs`) → C. Memory ops become `memb`/`memw` helpers; instrumentation points (`SAFEPOINT()`) are preserved. **This is where most translation bugs live.** The JIT run path uses the `PCSwitchRenderer` (dispatch-switch) renderer; the base structured renderer + `patterns.rs`/`ast.rs` back the readable-C output the unit tests exercise.
- `build_pipeline` (internal) — runs disassemble then ir_to_c on one input, emitting `<name>.c`; driven by the `saisei-jitc emit-c` / `jit-compile` subcommands. The JIT calls exactly this on each live memory segment.
- `generate_game_config` (in `saisei/`) — emits the per-game `GameConfig` C (program image path, PSP load segment, protected slots) from the one `<name>.json`. Carries no dispatch table and no entry symbol: the runtime takes the entry `cs:ip` from the MZ header and JITs from there.
...
*Freezing:* the end goal is to collect the JIT-discovered chunks and link them into a fully static native build (no runtime the reference/clang). The runtime dispatch tables (`GameConfig.binary_dispatch`, the `DispatchFn` ABI in `runtime/include/game_config.h`) are retained NULL-but-shaped for that future freeze to populate; today they are empty and every address routes through the JIT.
...
- **Bugs are in our shims or our generated C — never in the game; find them by reading OUR code against the x86/DOS/BIOS spec, not by studying the game's.** The games are real, correct, shipping binaries. The trap is tracing the game's own code (loader, decompressor, allocator) *to localize which shim is unfaithful* — still studying the game, still a waste: the game is not the variable, our model of x86/DOS/BIOS/HW is, and tracing the game only re-derives the faithful behavior you already know from the spec. Instead, name the corruption's signature, list the OUR-code surfaces (`runtime/` shims, `saisei-jitc/` codegen) that implement that operation, and read them against the spec for an unfaithful implementation or simplification — a library call standing in for an instruction's exact semantics, uncomputed flags, an "approximate"/"good enough" shortcut, an abort-on-unknown stub. Decode the game only to *confirm* a hypothesis already pinned to a named `file:function` in our code — never to discover the bug, and not via the generated game (`program.ir.json`, `build/<game>/jit/*.c` are the game re-expressed in C; reading them to follow what the game computes is game-tracing with extra steps, though reading a chunk to fix a codegen defect visible in the C is fine). (Example: `handle_les`/`handle_lds` hardcoded the mem-operand segment to DS, but `[bp+…]` defaults to SS — a codegen bug; likewise a block-copy shim's `memmove` silently broke `rep movs` overlap-replication semantics — a shim bug. Both found by reading our code, not the game.)
