...
pub struct cs_detail {
    // SAISEI ABI PATCH (rc2 -> 5.0.7): capstone 5.0.x grew MAX_IMPL_R_REGS 16->20
    // and inserted `writeback` before the arch union. Without this the union
    // (cs_x86) is read 8 bytes too early and op_count is garbage. cs_insn/cs_x86/
    // cs_x86_op are unchanged. Keep in sync with the linked libcapstone.
    ...
    pub groups_count: u8,
    #[doc = "< Instruction has writeback operands (added in capstone 5.0)."]
    pub writeback: bool,
